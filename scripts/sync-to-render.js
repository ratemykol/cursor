...
execSync(`pg_dump "${LOCAL_DB_URL}" --data-only --inserts > temp_local_export.sql`, { stdio: 'inherit' });
...
execSync(`psql "${RENDER_DB_URL}" < temp_local_export.sql`, { stdio: 'inherit' });

console.log('Cleaning up...');
fs.unlinkSync('temp_local_export.sql');

console.log('Render database synced from local successfully!');